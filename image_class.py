...
def PlistKeyFromImageArray(images, keyPath, missingKeyVal = 'raise'):
    # Returns a 1D array consisting of the values for the specified key path in the plist of each image
    # In future, this could be upgraded to return a 2D array if passed a list of key paths.
    # For now, the caller can just call this multiple times and fuse the results in to a 2D array, if desired.
    vals = []
    for im in images:
        try:
            val = im.frameKeyPath(keyPath)
        except:
            if missingKeyVal == 'raise':
                # TODO: we are not currently filling out im.path, by the look of it.
                # Need to do something about that
                print('Key path %s not found for image at %s' % (keyPath, im._path))
                raise
            else:
                val = missingKeyVal
        vals.append(val)
