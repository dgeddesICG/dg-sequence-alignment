...
from image_saving import DeleteAnyCachesInDir, ResaveMetadataAfterEditing
...
def EditOriginalPlists(fluorPath, fluorImages, key):
	...
	# DG : been having issue with Xcode and Stack viewer not liking phase being an np.float
        # Going to catch it and convert to native python type. Pretty sure its some interaction with my
        # plistlib or my Macbook is just cursed.
        if isinstance(fluorImages[i].phase, (np.float64, np.generic, np.ndarray)):
            tmpPhase = fluorImages[i].phase
            fluorImages[i].phase = float(tmpPhase.item())
        if isinstance(fluorImages[i].refFrame, (np.float64, np.generic, np.ndarray)):
            tmpRefFrame = fluorImages[i].refFrame
            fluorImages[i].refFrame = float(tmpRefFrame.item())
