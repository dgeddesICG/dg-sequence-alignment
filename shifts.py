...
def ShiftScoresForSequences(seqA, seqB, numSamplesPerPeriod, window1=None, window2=None, useFFT=True):
    ...
    # The shifts are evaluated in terms of the sum-squared-differences (which is minimised).
    # This matches Liebling, and is nice because it is equivalent to a cross-correlation,
    # which can be implemented quickly using an FFT
    if (useFFT == False):
        ...
        t4 = time.time()
        # This is mathematically equivalent to the sum-squared-differences.
        # I have left it like this for ease of comparison, but note that the first two terms
        # are constants that have no effect on the result. (They shouldn't take long to evaluate though)
